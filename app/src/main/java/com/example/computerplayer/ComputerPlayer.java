package com.example.computerplayer;

import java.util.Random;
import android.graphics.Point;
import com.example.game.*;
import com.example.scoretable.ScoreTable;
/**
 * AI类
 * 参考了别人的算法写出来的
...
public class ComputerPlayer {
	...
	private int[][] computerMap = new int[GameView.ROWS][GameView.COLS];//电脑棋盘
	private int[][] playerMap = new int[GameView.ROWS][GameView.COLS];//玩家棋盘
	// 电脑的棋子颜色
	...
	private Point getBestPoint() {
		...
		Point pcPoint = new Point(-1, -1);//电脑最大得分点
		Point playerPoint = new Point();//玩家最大得分点
		// 分别选出pc估分和玩家估分的最大值
		for (int r = 0; r < GameView.ROWS; r++) {
			for (int c = 0; c < GameView.COLS; c++) {
				// 选出电脑估分的最大值
	...
	private int getValue(int r, int c, ChessType chessType) {
		final int directions=4;//四个记分方向
		int[] dir = new int[directions];//该位置的棋子四个方向上的得分
